...
def convert(opt):
    """
    Converts an ONNX model to TensorRT engine format.

    Args:
        opt (argparse.Namespace): Parsed arguments with the following attributes:
            - weight (str): Path to the ONNX model (.onnx).
            - gpu_num (int): GPU number to use.
            - verbose (bool): Flag to enable verbose logging.
            - workspace (int): Workspace size in GB for TensorRT.
            - fp16 (bool): Use FP16 precision if enabled.

    Raises:
        AssertionError: If the ONNX model file does not exist.
        RuntimeError: If the ONNX file cannot be parsed.

    Returns:
        None: Saves the TensorRT engine to disk.
    """
